...
|------|------|------|
| `Exit` | `Exit` | 停止 Bot |
| `help` | `help` / `help <CommandName>` | 列出所有指令，或查看某指令的详细用法 |
...
1. 类的完整名称（含命名空间）必须为 `Plugins.Main.MainBehavior`
2. 实现 `BotMain.Plugin.IPluginMain` 接口
3. 标注 `[PluginEntry(pluginName, pluginDes, pluginVersion)]` Attribute
...
public class MainBehavior : IPluginMain
{
    public void Init()
    {
        // 注册事件、初始化资源
